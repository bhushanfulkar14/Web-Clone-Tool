      ...
      <svg viewBox="0 0 400 200" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        <defs>
          <linearGradient id="bg1" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="#064e3b" /><stop offset="100%" stopColor="#0f172a" />
          </linearGradient>
          <linearGradient id="bar1" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#10b981" /><stop offset="100%" stopColor="#064e3b" />
          </linearGradient>
        </defs>
        {[40,80,120,160].map(y=><line key={y} x1="30" y1={y} x2="370" y2={y} stroke="#ffffff08" strokeWidth="1"/>)}
        {[{x:50,h:90},{x:95,h:120},{x:140,h:65},{x:185,h:145},{x:230,h:100},{x:275,h:80},{x:320,h:130}].map((b,i)=>(
          <rect key={i} x={b.x} y={175-b.h} width="28" height={b.h} rx="4" fill="url(#bar1)" opacity="0.85"/>
        ))}
      ...
      <svg viewBox="0 0 400 200" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        <defs>
          ...
          <radialGradient id="brainGlow" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#7c3aed" stopOpacity="0.6"/><stop offset="100%" stopColor="#1e1b4b" stopOpacity="0"/>
          </radialGradient>
        </defs>
        ...
        {[{cx:90,cy:55,label:"Symptoms"},{cx:90,cy:135,label:"Triage"},{cx:310,cy:55,label:"Diagnosis"},{cx:310,cy:135,label:"Reports"},{cx:200,cy:170,label:"DB"}].map((n,i)=>(
          <g key={i}>
  ...
  },
  {
    ...
    visual: (
      <svg viewBox="0 0 400 200" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        {[{x:20,val:"87%",label:"Retention"},{x:120,val:"12.4%",label:"Attrition"},{x:220,val:"4.2K",label:"Headcount"},{x:320,val:"78%",label:"Satisfaction"}].map((k,i)=>(
          <g key={i}>
        ...
        {[{x:40,h1:70,h2:30},{x:90,h1:90,h2:20},{x:140,h1:50,h2:50},{x:190,h1:110,h2:15},{x:240,h1:75,h2:35},{x:290,h1:95,h2:25},{x:340,h1:60,h2:45}].map((b,i)=>(
          <g key={i}>
      ...
      <svg viewBox="0 0 400 200" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        <defs>
          <linearGradient id="bg4" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="#431407" /><stop offset="100%" stopColor="#0f172a" />
          </linearGradient>
        </defs>
        ...
        {["Topic: Machine Learning","Difficulty: Medium","Questions: 10 · Marks: 50"].map((t,i)=>(
          <g key={i}><rect x="26" y={42+i*16} width="150" height="12" rx="3" fill="#f9731615"/>
          <text x="31" y={51+i*16} fill="#94a3b8" fontSize="7" fontFamily="monospace">{t}</text></g>
        ))}
        <text x="200" y="68" textAnchor="middle" fill="#f97316" fontSize="18" fontFamily="monospace">→</text>
        ...
        {[{q:"Q1. Define overfitting.",m:"2M",c:"#86efac"},{q:"Q2. Implement k-NN.",m:"5M",c:"#fde68a"},{q:"Q3. Compare SVM vs RF.",m:"8M",c:"#f9a8d4"},{q:"Q4. Bias-variance tradeoff.",m:"10M",c:"#93c5fd"}].map((q,i)=>(
          <g key={i}>
            ...
            <text x="361" y={55+i*24} textAnchor="middle" fill={q.c} fontSize="6" fontFamily="monospace" fontWeight="bold">{q.m}</text>
          </g>
        ))}
      </svg>
    ),
      ...
      <svg viewBox="0 0 400 200" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        <defs>
          <linearGradient id="bg5" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="#0c2a3a" /><stop offset="100%" stopColor="#0f172a" />
          </linearGradient>
        </defs>
        ...
        {[{x:30,o:155,c:130,h:120,l:165},{x:58,o:130,c:140,h:122,l:148},{x:86,o:140,c:115,h:108,l:150},{x:114,o:115,c:128,h:108,l:135},{x:142,o:128,c:108,h:100,l:135},{x:170,o:108,c:125,h:100,l:130},{x:198,o:125,c:105,h:96,l:130},{x:226,o:105,c:118,h:98,l:125},{x:254,o:118,c:95,h:88,l:122},{x:282,o:95,c:110,h:86,l:118},{x:310,o:110,c:88,h:80,l:115},{x:338,o:88,c:102,h:78,l:108}].map((c,i)=>{
          const isUp=c.c<c.o; const color=isUp?"#22d3ee":"#f87171";
          const top=Math.min(c.o,c.c); const bot=Math.max(c.o,c.c);
          return(<g key={i}><line x1={c.x+10} y1={c.h} x2={c.x+10} y2={c.l} stroke={color} strokeWidth="1.5" opacity="0.7"/><rect x={c.x} y={top} width="20" height={Math.max(bot-top,2)} rx="2" fill={color} opacity="0.85"/></g>);
        })}
      </svg>
    ),
  },
      ...
      <svg viewBox="0 0 400 200" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        <defs>
          <linearGradient id="bg6" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="#4a0520" /><stop offset="100%" stopColor="#0f172a" />
          </linearGradient>
        </defs>
        ...
        {[{name:"Arjun S.",time:"09:01",present:true},{name:"Priya M.",time:"09:03",present:true},{name:"Rahul K.",time:"09:12",present:true},{name:"Sneha P.",time:"09:18",present:true},{name:"Vikram T.",time:"—",present:false}].map((r,i)=>(
          <g key={i}>
  ...
  return (
    <section id="projects" className="py-24 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
            ...
            >
              <div className={`relative h-48 bg-gradient-to-br ${project.color} overflow-hidden`}>
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 transition-all duration-300 flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100 backdrop-blur-[2px]">
                  <a href="#" className="w-11 h-11 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-all hover:scale-110" onClick={e=>e.preventDefault()}>
              ...
              <div className="p-5 flex flex-col flex-1">
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {project.tags.map(tag => (
                    <span key={tag} className="text-[10px] px-2.5 py-1 rounded-full bg-white/5 border border-white/10 text-gray-300 font-medium">
                      {tag}
                    </span>
                  ))}
                </div>
                <div className="flex items-center gap-2 pt-3 border-t border-white/5">
                  ...
                  <span className="ml-auto text-[10px] text-gray-600 bg-white/5 px-2 py-0.5 rounded-full">NexGen Student</span>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        {/* Bottom CTA — fixed to use onNavigate */}
        <motion.div
